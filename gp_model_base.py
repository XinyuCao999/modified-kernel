...
def train_GP(input_data,output_data,model_type,print_flag=True,**kwargs):
    ...
    if model_type=="RBF_zeroprior":
        ...
        model, likelihood = train_multi_fidelity_gp(input_data, output_data, print_flag, machanistic_fun, bounds=bounds)
...
def train_reconstructed_gp(input_data, output_data,  print_flag, machanistic_fun, training_iter=100, learning_rate=0.1,return_all_flag=False,decay_factor=0.9):
    """
    Train a Gaussian Process model with:
...
class _HighFidelityWrapper(Model):
    """
    Present a multi-fidelity model as a plain d-dimensional model evaluated at
    the highest fidelity. It appends the target-fidelity column to any query X
    before calling the wrapped model, so the existing d-dimensional EI/UCB +
    optimize_acqf loop can use it unchanged.
    """

    def __init__(self, mf_model, target_fidelity=1.0, fidelity_dim=None):
        super().__init__()
    ...
    def num_outputs(self):
        return self.mf_model.num_outputs

    @property
    def batch_shape(self):
        return self.mf_model.batch_shape

    def posterior(self, X, output_indices=None, observation_noise=False,
                  posterior_transform=None, **kwargs):
        s = torch.full((*X.shape[:-1], 1), self.target_fidelity,
                       dtype=X.dtype, device=X.device)
        X_aug = torch.cat([X, s], dim=-1)
        return self.mf_model.posterior(
            X_aug, output_indices=output_indices,
            observation_noise=observation_noise,
            posterior_transform=posterior_transform, **kwargs,
        )


def train_multi_fidelity_gp(input_data, output_data, print_flag, machanistic_fun,
                            bounds=None, n_low=0, para_output=False):
    """
    BoTorch multi-fidelity GP baseline: SingleTaskMultiFidelityGP with a
    linear-truncated fidelity kernel.

    Fidelity is added as an extra input dimension s:
        s = 0  -> low fidelity  = the mechanistic model `machanistic_fun`
        s = 1  -> high fidelity = the real observations

    Low-fidelity design (co-located / nested): the mechanistic model is evaluated
    at the SAME locations as the high-fidelity points, so the fidelity kernel can
    directly learn the local prior-to-truth bias at each observed point. This is
    the standard nested multi-fidelity design; feeding extra spread-out low-fi
    points instead re-injects the (possibly biased) prior in regions with no
    high-fi correction and degrades the high-fidelity posterior. `n_low` extra
    global LHS low-fi points can still be added (default 0).

    The returned model is wrapped so it behaves like an ordinary d-dimensional
    model that always predicts at the highest fidelity (s = 1), letting the
    existing EI/UCB + optimize_acqf loop use it unchanged.

    Args:
        input_data (array-like): high-fidelity inputs of shape [n, d]
        output_data (array-like): high-fidelity targets of shape [n] or [n, 1]
        machanistic_fun (callable): low-fidelity mechanistic model, maps [n, d] -> [n]
        bounds (np.ndarray): input-domain bounds of shape [d, 2] (required)
        n_low (int): number of EXTRA global LHS low-fi points (default 0)

    Returns:
        model (wrapped, d-dimensional), likelihood (both in eval mode)
    """
    if bounds is None:
        raise ValueError("train_multi_fidelity_gp requires `bounds` for low-fidelity sampling.")

    bounds = np.asarray(bounds, dtype=float)
    d = bounds.shape[0]

    # high-fidelity data (s = 1)
    if isinstance(input_data, torch.Tensor):
        X_high = input_data.clone().detach().float()
    else:
        X_high = torch.tensor(input_data, dtype=torch.float32)
    if isinstance(output_data, torch.Tensor):
        y_high = output_data.clone().detach().float().reshape(-1, 1)
    else:
        y_high = torch.tensor(output_data, dtype=torch.float32).reshape(-1, 1)

    # low-fidelity data (s = 0): co-located with the high-fidelity points, plus
    # optional extra global LHS points (n_low, default 0).
    X_low_parts = [X_high.clone()]
    if n_low > 0:
        lhs = qmc.LatinHypercube(d=d, seed=123)      # fixed seed -> reproducible
        X_extra = qmc.scale(lhs.random(n=n_low), bounds[:, 0], bounds[:, 1])
        X_low_parts.append(torch.tensor(X_extra, dtype=torch.float32))
    X_low = torch.cat(X_low_parts, dim=0)
    y_low = machanistic_fun(X_low).reshape(-1, 1).float()

    # augment each block with its fidelity column and stack
    fid_dim = d
    X_low_aug = torch.cat([X_low, torch.zeros(X_low.shape[0], 1)], dim=-1)
    X_high_aug = torch.cat([X_high, torch.ones(X_high.shape[0], 1)], dim=-1)
    train_X = torch.cat([X_low_aug, X_high_aug], dim=0)
    train_Y = torch.cat([y_low, y_high], dim=0)

    # normalization bounds: x-dims from `bounds`, fidelity in [0, 1]
    x_bounds = torch.tensor(bounds.T, dtype=torch.float32)                     # (2, d)
    aug_bounds = torch.cat([x_bounds, torch.tensor([[0.0], [1.0]])], dim=1)    # (2, d+1)

    if print_flag:
        print("---------------------fitting BoTorch multi-fidelity GP...------------------------")

    model = SingleTaskMultiFidelityGP(
        train_X, train_Y,
        data_fidelities=[fid_dim],
        outcome_transform=Standardize(m=1),
        input_transform=Normalize(d=d + 1, bounds=aug_bounds),
    )
    ...
    wrapped = _HighFidelityWrapper(model, target_fidelity=1.0, fidelity_dim=fid_dim)

    if print_flag:
        print("------------------------end fitting---------------------------")

    if para_output:
        return wrapped, model.likelihood, None
    else:
        return wrapped, model.likelihood
